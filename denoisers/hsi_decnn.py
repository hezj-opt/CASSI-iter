...
class hsicnn_denoiser(nn.Module):
    ...
    def forward(self, x):
        ...
        Nsigma = torch.full((1, 1, 1, 1), self.sigma[self.iter] / 255.).type_as(x)

        with torch.no_grad():
            for i in range(C):
                ch = np.array([i-3, i-2, i-1, i, i+1, i+2, i+3])
                ch[ch<0]=0
                ch[ch>=C]=C-1
                
                out = self.model(x[:, ch, :, :], Nsigma)
                tem.append(out)
                
        self.iter = self.iter + 1
        
        res = torch.cat(tem, dim=1)
        res = res.squeeze(0)
